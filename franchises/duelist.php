	...
	$sql = "
		...
		INNER JOIN 
		 (SELECT St_Name,p1.*
				FROM student_info
				INNER JOIN 
				(	SELECT student_id,course_id,regno,SUM(payment_amt) AS paidamt,course_fee,complete_month,complete_year
					FROM
					(SELECT `pursuing_course`.*,payment.payment_amt 
					FROM pursuing_course
					LEFT JOIN payment
					ON pursuing_course.student_id=payment.student_id
					WHERE pursuing_course.current_status='PURSUING' AND pursuing_course.franchise_id='{$_SESSION['franchise_id']}' )p2 
					GROUP BY p2.student_id,p2.course_id
				)p1
				ON student_info.slno=p1.student_id)p4
				on p4.course_id=courses.id
			";
